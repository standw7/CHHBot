...
const logger = pino({ name: 'moneypuck' });
...
export async function getMoneyPuckSkaters(teamCode: string): Promise<MoneyPuckSkater[] | null> {
  // Check cache
...
function parseCSVEfficient(csv: string): MoneyPuckSkater[] {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (values[idx.situation] !== 'all') continue;
...
function filterByTeam(skaters: MoneyPuckSkater[], teamCode: string): MoneyPuckSkater[] {
