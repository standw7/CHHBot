...
function parseTime(input, timezone) {
    ...
    if (absNumMatch) {
        ...
        if (ampm === 'am' && hour === 12)
            hour = 0;
        const date = luxon_1.DateTime.fromObject({ year, month, day, hour, minute: min, second: 0 }, { zone: timezone });
        if (!date.isValid)
            return null;
    ...
    if (absNameMatch) {
        ...
        if (ampm === 'am' && hour === 12)
            hour = 0;
        const date = luxon_1.DateTime.fromObject({ year: now.year, month, day, hour, minute: min, second: 0 }, { zone: timezone });
        if (!date.isValid)
            return null;
        return { date, relative: date.toFormat('MMM d \'at\' h:mm a') };
    }
