...
function registerMessageHandler(client) {
    client.on('messageCreate', async (message) => {
        ...
        try {
            ...
            try {
                await message.reply(`Something went wrong with \`!${command}\`. Check bot logs.`);
            }
            catch { }
            ;
        }
...
async function handlePrefixHelp(message) {
    ...
    let currentPage = 0;
    const buildButtons = (page) => new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder()
...
async function handlePrefixRewards(message) {
...
function describeCandidates(players) {
    return players.map(p => `${p.firstName} ${p.lastName}${p.sweaterNumber ? ` (#${p.sweaterNumber})` : ''}`).join(', ');
}
async function handlePrefixFollow(message, query) {
    ...
    const teamCode = (0, queries_js_1.getGuildConfig)(message.guild.id)?.primary_team ?? 'UTA';
...
async function handlePrefixFollowing(message) {
    ...
    const lines = [];
    if (players.length > 0)
        lines.push(`**Players:** ${players.map(f => f.playerName).join(', ')}`);
    if (members.length > 0)
        lines.push(`**Members:** ${members.map(id => `<@${id}>`).join(', ')}`);
...
async function handleFollowMember(message, target) {
    ...
    if (check === 'ok')
        addMemberFollow(guildId, message.author.id, target.id);
    await message.reply({ content: replies[check], allowedMentions: { parse: [] } });
}
async function handleFollowOptOut(message, optOut) {
...
async function handlePrefixFollowers(message) {
...
async function handlePrefixHof(message, args) {
    ...
    if (sub === 'update') {
        ...
        for (const entry of entries) {
            ...
            if (deleted % 5 === 0) {
                try {
                    await statusMsg.edit(`Deleting old HOF posts... (${deleted}/${entries.length})`);
                }
                catch { }
            }
        ...
        for (let i = 0; i < entries.length; i++) {
            ...
            try {
                ...
                let origMessage;
                try {
                    origMessage = await origChannel.messages.fetch(entry.original_message_id);
                }
                catch {
                    skipped++;
                    continue;
            ...
            if ((i + 1) % 5 === 0 || i === entries.length - 1) {
                try {
                    await statusMsg.edit(`Resending HOF posts... (${i + 1}/${entries.length})`);
                }
                catch { }
            }
